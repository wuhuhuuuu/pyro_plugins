...
class Sticker:
    ...
    def get_config(self) -> str:
        ...
        return (
            ...
            f"使用命令 <code>,{alias_command('s')} to 贴纸包名</code> 短暂自定义保存贴纸包"
        )


@listener(
    command="s",
    parameters="[贴纸包名/to 贴纸包名/cancel]",
...
async def sticker(message: Message):
    ...
    if len(message.parameter) == 2 and message.parameter[0] == "to":
        one_sticker.sticker_set = message.parameter[1]
        try:
            await one_sticker.check_pack_full()
        except NoStickerSetNameError:
            pass
